...
//  lis3mdl_success2 = lis3mdl.begin_I2C(0x1C);
  ...
  if (!(lsm6ds_success2 && lis3mdl_success2)) {
    while (1) {
      delay(10);
    }
  }
  ...
  Serial.print("Accelerometer data rate set to: ");
  switch (lsm6ds2.getAccelDataRate()) {
  case LSM6DS_RATE_SHUTDOWN:
    Serial.println("0 Hz");
    break;
  case LSM6DS_RATE_12_5_HZ:
  ...
  Serial.print("Gyro data rate set to: ");
  switch (lsm6ds2.getGyroDataRate()) {
  case LSM6DS_RATE_SHUTDOWN:
    Serial.println("0 Hz");
    break;
  case LSM6DS_RATE_12_5_HZ:
  ...
  Serial.print("Magnetometer performance mode set to: ");
  switch (lis3mdl2.getPerformanceMode()) {
    case LIS3MDL_LOWPOWERMODE: Serial.println("Low"); break;
    case LIS3MDL_MEDIUMMODE: Serial.println("Medium"); break;
    case LIS3MDL_HIGHMODE: Serial.println("High"); break;
    case LIS3MDL_ULTRAHIGHMODE: Serial.println("Ultra-High"); break;
  ...
  Serial.print("Magnetometer operation mode set to: ");
  // Single shot mode will complete conversion and go into power down
  switch (lis3mdl2.getOperationMode()) {
    case LIS3MDL_CONTINUOUSMODE: Serial.println("Continuous"); break;
    case LIS3MDL_SINGLEMODE: Serial.println("Single mode"); break;
    case LIS3MDL_POWERDOWNMODE: Serial.println("Power-down"); break;
  }

  lis3mdl2.setIntThreshold(500);
  lis3mdl2.configInterrupt(false, false, true, // enable z axis
                          true, // polarity
                          false, // don't latch
                          true); // enabled!
...
//  }
  ...
  dataString += ",";
  dataString += String(accel2.acceleration.z);
  dataString += ",";
  dataString += String(gyro2.gyro.x);
  dataString += ",";
  dataString += String(gyro2.gyro.y);
  dataString += ",";
  dataString += String(gyro2.gyro.z);
  dataString += ",";
  dataString += String(mag2.magnetic.x);
  dataString += ",";
  dataString += String(mag2.magnetic.y);
  dataString += ",";
